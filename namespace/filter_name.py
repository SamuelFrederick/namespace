...
class FilterName:
    """
    Class to filter possible name matches using cosine similarity nearest neighbors. 
    ...
    """
    def __init__(self, embedding_model = 'SamFrederick/namespace500k', return_sim = False, **kwargs):
        self.emb_mod = EmbedName(
            embedding_model = embedding_model, 
            tokenizer = kwargs.get('tokenizer', 'roberta-large'), 
    ...
    def filter_names(
        ...
        if self.k == 1:
            ...
            if self.return_sim:
                all_out = [(name1[i], n2[i], float(out.values[i])) for i in range(len(name1))]
            else:
                all_out = [(name1[i], n2[i]) for i in range(len(name1))]
        else:
            all_out = []
